...
class EventManager(models.QuerySet):
    ...
    def query_db_by_action(
        self, action, order_by="-timestamp", start=None, end=None
    ) -> models.QuerySet:
        events = self
        any_step = Q()
    ...
    def query_retention(
        self, filters, team, start_entity: Optional[Entity] = None
    ) -> dict:
        ...
        if entity.type == TREND_FILTER_TYPE_EVENTS:
            events = Event.objects.filter_by_event_with_people(
                event=entity.id, team_id=team.id
            )
        ...
        for row in data:
            ...
            cache_key = generate_cache_key(
                "{}{}{}".format(random_key, str(round(row.first_date, 0)), str(team.pk))
            )
            cache.set(
    ...
    def create(self, site_url: Optional[str] = None, *args: Any, **kwargs: Any):
        with transaction.atomic():
            ...
            if not settings.ASYNC_EVENT_ACTION_MAPPING:
                ...
                Action.events.through.objects.bulk_create(
                ...
                if should_post_to_slack and team and team.slack_incoming_webhook:
                    celery.current_app.send_task(
                        "posthog.tasks.slack.post_event_to_slack", (event.pk, site_url)
                    )
